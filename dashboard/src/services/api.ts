...
export type AiApiHttpMethod = 'GET' | 'POST';
...
export const aiApi = {
  ...
  updateConfig: (config: UpdateAiConfigPayload) =>
    request<AiConfig>('/ai/config', {
      method: 'PUT',
      body: JSON.stringify(config),
    }),
  getKnowledge: () => request<AiKnowledge>('/ai/knowledge'),
  updateKnowledge: (content: string) =>
    request<AiKnowledge>('/ai/knowledge', {
      method: 'PUT',
      body: JSON.stringify({ content }),
    }),
  getInteractions: (limit = 100) => request<AiInteraction[]>(`/ai/interactions?limit=${limit}`),
  clearInteractions: () => request<void>('/ai/interactions', { method: 'DELETE' }),
  listApis: () => request<AiApiLink[]>('/ai/apis'),
  createApi: (payload: CreateAiApiLinkPayload) =>
    request<AiApiLink>('/ai/apis', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
  updateApi: (id: string, payload: UpdateAiApiLinkPayload) =>
    request<AiApiLink>(`/ai/apis/${id}`, {
      method: 'PUT',
      body: JSON.stringify(payload),
    }),
  deleteApi: (id: string) => request<void>(`/ai/apis/${id}`, { method: 'DELETE' }),
  testApi: (id: string) =>
    request<AiApiTestResult>(`/ai/apis/${id}/test`, { method: 'POST' }),
};
