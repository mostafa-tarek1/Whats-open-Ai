...
import {
  ...
  pluginsApi,
  aiApi,
  type CreateAiApiLinkPayload,
  type UpdateAiApiLinkPayload,
  type Webhook,
} from '../services/api';
...
export const queryKeys = {
  ...
  aiKnowledge: ['ai', 'knowledge'] as const,
  aiInteractions: (limit: number) => ['ai', 'interactions', limit] as const,
  aiApis: ['ai', 'apis'] as const,
...
export function useUpdateAiConfigMutation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiConfig });
    },
  });
}

export function useAiKnowledgeQuery() {
  return useQuery({
    queryKey: queryKeys.aiKnowledge,
    queryFn: aiApi.getKnowledge,
    staleTime: 30_000,
  });
}

export function useUpdateAiKnowledgeMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: aiApi.updateKnowledge,
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiStatus });
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiConfig });
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiKnowledge });
    },
  });
}

export function useAiInteractionsQuery(limit = 100) {
  return useQuery({
    queryKey: queryKeys.aiInteractions(limit),
    queryFn: () => aiApi.getInteractions(limit),
    staleTime: 10_000,
    refetchInterval: 15_000,
  });
}

export function useClearAiInteractionsMutation(limit = 100) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: aiApi.clearInteractions,
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiInteractions(limit) });
    },
  });
}

export function useAiApisQuery() {
  return useQuery({
    queryKey: queryKeys.aiApis,
    queryFn: aiApi.listApis,
    staleTime: 30_000,
  });
}

export function useCreateAiApiMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: CreateAiApiLinkPayload) => aiApi.createApi(payload),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiApis });
    },
  });
}

export function useUpdateAiApiMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (params: { id: string; payload: UpdateAiApiLinkPayload }) =>
      aiApi.updateApi(params.id, params.payload),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiApis });
    },
  });
...
export function useDeleteAiApiMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => aiApi.deleteApi(id),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.aiApis });
    },
  });
...
export function useTestAiApiMutation() {
  return useMutation({
    mutationFn: (id: string) => aiApi.testApi(id),
  });
}
