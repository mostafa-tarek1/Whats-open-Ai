...
const allNavItems = [
  ...
  { to: '/ai', icon: Bot, key: 'ai' as const, adminOnly: true },
...
export function Layout({ onLogout, userRole }: LayoutProps) {
  ...
  const detectedLang = (i18n.resolvedLanguage || i18n.language || 'ar').split('-')[0];
  const currentLang = supportedLanguages.includes(detectedLang as SupportedLanguage)
    ? (detectedLang as SupportedLanguage)
    : 'ar';
  const cycleLanguage = () => {
  ...
  const languageLabel = currentLang === 'ar' ? 'عربي' : 'EN';
    ...
    <div className="layout">
      {isMobile && (
        <header className="mobile-header">
          ...
          <div className="mobile-brand">
            <img src="/noxus-logo.png" alt="NOXUS AI" className="mobile-wordmark" />
          </div>
          <div style={{ width: 40 }} />
        </header>
      )}
      ...
      >
        <div className="sidebar-header">
          {isCollapsed ? (
            <span className="noxus-mark" aria-label="NOXUS AI">
              N
            </span>
          ) : (
            <img src="/noxus-logo.png" alt="NOXUS AI" className="sidebar-wordmark" />
          )}
          {!isCollapsed && (
