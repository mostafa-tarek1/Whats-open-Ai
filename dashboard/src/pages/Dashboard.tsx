...
export function Dashboard() {
  ...
  const aiReady = aiStatus?.enabled && aiStatus.configured && aiStatus.knowledgeLoaded;
  const aiState = aiReady ? 'ready' : aiStatus?.enabled ? 'attention' : 'off';
  const aiStateLabel = aiReady
    ? t('dashboard.ai.ready')
    : aiStatus?.enabled
      ? t('dashboard.ai.needsSetup')
      : t('dashboard.ai.off');
    ...
    <div className="dashboard">
      ...
      <section className="ai-command-panel">
        <div className="ai-command-brand">
          <span className="ai-kicker">NOXUS AI</span>
          <h2>{t('dashboard.ai.title')}</h2>
          <p>{t('dashboard.ai.subtitle')}</p>
        </div>
        <div className="ai-command-grid">
          <div className="ai-command-item">
            <Bot size={20} />
            <span>{t('dashboard.ai.model')}</span>
            <strong>{aiStatus?.model || 'gemini-2.5-flash'}</strong>
          </div>
          <div className="ai-command-item">
            <Database size={20} />
            <span>{t('dashboard.ai.knowledge')}</span>
            <strong>{aiStatus?.knowledgeLoaded ? t('common.active') : t('common.inactive')}</strong>
          </div>
          <div className={`ai-command-item ${aiState}`}>
            <Radio size={20} />
            <span>{t('dashboard.ai.autoReply')}</span>
