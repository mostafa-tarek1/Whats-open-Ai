export default () => ({
  ...
  ai: {
    enabled: process.env.AI_REPLY_ENABLED === 'true',
    provider: process.env.AI_PROVIDER || 'gemini',
    geminiApiKey: process.env.GEMINI_API_KEY || '',
    geminiModel: process.env.GEMINI_MODEL || 'gemini-2.5-flash',
    knowledgePath: process.env.AI_KNOWLEDGE_PATH || './data/knowledge/noxus.md',
    knowledgeMaxChars: parseInt(process.env.AI_KNOWLEDGE_MAX_CHARS || '20000', 10),
    maxReplyChars: parseInt(process.env.AI_REPLY_MAX_CHARS || '900', 10),
    cooldownMs: parseInt(process.env.AI_REPLY_COOLDOWN_MS || '30000', 10),
    includeGroups: process.env.AI_REPLY_INCLUDE_GROUPS === 'true',
    temperature: parseFloat(process.env.AI_TEMPERATURE || '0.35'),
    systemPrompt:
      process.env.AI_SYSTEM_PROMPT ||
      'You are NOXUS AI customer support on WhatsApp. You are accurate, concise, and grounded only in company-provided knowledge.',
  },
});
